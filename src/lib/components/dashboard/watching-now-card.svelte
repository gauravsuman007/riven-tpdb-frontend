        ...
        sessions.map((session) => {
            const progress =
                ...
                session.durationSeconds > 0
                    ? Math.max(0, Math.min(100, (session.positionSeconds / session.durationSeconds) * 100))
                    : 0;
            const remaining =
                session.positionSeconds !== null &&
                session.durationSeconds !== null &&
            ...
            return {
                ...
                playbackVariant:
                    playbackTone[session.playbackState.toLowerCase() as keyof typeof playbackTone] ??
                    "secondary",
                methodLabel:
                    playbackMethod[session.playbackMethod.toLowerCase() as keyof typeof playbackMethod] ??
                    "Unknown",
                ...
                showClient:
                    !!session.deviceName &&
                    !!session.clientName &&
                    session.deviceName.trim().toLowerCase() !== session.clientName.trim().toLowerCase(),
    ...
    <Card.Root>
        <Card.Header class="pb-2">
            <div class="flex items-center justify-between gap-3">
                <Card.Title class="text-sm font-medium text-neutral-300">Watching Now</Card.Title>
                            ...
                            <div class="flex gap-2.5">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-start justify-between gap-2">
                                        ...
                                        <Badge variant={session.playbackVariant} class="rounded-md px-1.5 py-0 text-[10px]">
                                            {session.playbackLabel}
                                        </Badge>
                                    </div>

                                    <div class="mt-1.5 flex flex-wrap gap-x-2 gap-y-1 text-[10px] text-neutral-500">
                                        <span class="truncate">{session.userName ?? "Unknown user"} on {session.server}</span>
                                    ...
                                    <div class="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-neutral-500">
                                        <span class="truncate">
                                            ...
                                            <span class="ml-1 text-neutral-200">{session.deviceValue}</span>
                                        </span>
                                        {#if session.showClient}
                                            <span class="truncate">
                                                <span class="text-neutral-400">Client</span>
                                                <span class="ml-1 text-neutral-200">{session.clientName}</span>
                                            </span>
                                        {/if}
                                    </div>
