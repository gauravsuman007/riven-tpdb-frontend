<script lang="ts">
    ...
    /*
        The profile is a tab on the settings page now, so its data arrives as
        part of that page's. Typed loosely on purpose: a component under
        `$lib` cannot read a route's generated `./$types`, and restating
        four superforms and better-auth's account list here by hand would be
        a copy that goes stale silently. `loadProfile` in
        `$lib/server/profile.ts` is what actually defines this shape.
    */
...
<div class="flex flex-col">
    <h1 class="text-2xl font-bold tracking-tight">{data.user.name}'s Profile</h1>
